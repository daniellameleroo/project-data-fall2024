...
public class Estadio {
    ...
    private void initializeSeats() {
        for (int i = 1; i <= 500; i++) {
            avaiableSeatsField.add(new Asiento("Field Level", i / 10 + 1, i, false));
        }
        for (int i = 1; i <= 1000; i++) {
            avaiableSeatsMain.add(new Asiento("Main Level", i / 20 + 1, i, false));
        }
        for (int i = 1; i <= 2000; i++) {
            avaiableSeatsGrandstand.add(new Asiento("Grandstand Level", i / 30 + 1, i, false));
        }
    }
